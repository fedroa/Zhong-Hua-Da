/* =====================================================================
   ...
   dan form order via WhatsApp + pencatatan otomatis ke Google Sheets.

   ⚠️ PENTING UNTUK ADMIN — atur sebelum upload ke GitHub:
   1. ADMIN_WA            → nomor WhatsApp bisnismu (format 62xxxxxxxxxx)
   ...
   percobaan iseng, tapi bukan pengaman mutlak — selalu cek ulang harga
...
/* ---------------------------------------------------------------------
   ...
   Fire-and-forget: dikirim dengan mode 'no-cors' karena Google Apps
   Script Web App tidak mengirim header CORS untuk dibaca balik oleh
   JS. Data tetap masuk ke sheet walau kita tidak bisa membaca respons
   di sisi browser — lihat google-apps-script.gs untuk kode servernya.
--------------------------------------------------------------------- */
function logOrderToSheet(payload) {
  ...
  fetch(SHEET_WEBHOOK_URL, {
    ...
    // Diam-diam gagal saja — jangan sampai memblokir pesanan WhatsApp.
    console.warn("Gagal mencatat pesanan ke spreadsheet.");
  });
}

/* ---------------------------------------------------------------------
   ORDER FORM SUBMIT → CATAT KE SHEET + BUKA WHATSAPP
--------------------------------------------------------------------- */
function handleOrderSubmit(e) {
  ...
  const lines = [
    "Halo Zhong Hua Da 中华达, saya mau titip barang:",
    "",
    `Nama: ${nama}`,
    `WhatsApp: ${wa}`,
    `Alamat: ${alamat}`,
    ...
    ...realItems.map(
      (it, i) =>
        `${i + 1}. ${it.name} (${it.category.label}) — ¥${it.priceOrigin} x ${it.qty}, ${it.weight}kg/pcs`
    ),
  ];

  if (link) lines.push("", "Link produk:", link);
  if (catatan) lines.push("", `Catatan: ${catatan}`);

  if (realItems.length) {
    lines.push(
      "",
      "Estimasi dari kalkulator situs:",
      `- Harga barang: ${rupiah(result.totalBarangJual)}`,
      `- Ongkir: ${rupiah(result.totalOngkirJual)}`,
      `- Bea & pajak impor: ${rupiah(result.totalPajak)}`,
      `- Fee jastip: ${rupiah(result.totalFee)}`,
      `- Total tagihan: ${rupiah(result.totalTagihan)}`
    );
  }

  const text = encodeURIComponent(lines.join("\n"));
  window.open(`https://wa.me/${ADMIN_WA}?text=${text}`, "_blank", "noopener");
}
