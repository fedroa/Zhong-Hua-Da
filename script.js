...
const JAGO_ACCOUNT_NUMBER = "100995200459"; // TODO: ganti dengan nomor rekening Bank Jago-mu
const JAGO_ACCOUNT_NAME = "FEDRO ANDREANTO"; // TODO: ganti dengan nama sesuai buku tabungan
...
function showPaymentConfirmation(orderCode, totalTagihan) {
  ...
  // Kode pesanan muncul di dua tempat pada modal (header & langkah transfer)
  document.getElementById("payment-order-code").textContent = orderCode;
  document.getElementById("payment-order-code-2").textContent = orderCode;

  document.getElementById("payment-amount").textContent = rupiah(totalTagihan);

  // Rekening BCA
  document.getElementById("payment-bca-number").textContent = BCA_ACCOUNT_NUMBER;
  document.getElementById("payment-bca-name").textContent = BCA_ACCOUNT_NAME;

  // Rekening Bank Jago
  document.getElementById("payment-jago-number").textContent = JAGO_ACCOUNT_NUMBER;
  document.getElementById("payment-jago-name").textContent = JAGO_ACCOUNT_NAME;
...
function openPaymentModal() {
  ...
  // fokuskan tombol tutup untuk aksesibilitas
  const closeBtn = document.getElementById("payment-modal-close");
  if (closeBtn) closeBtn.focus();
}

function closePaymentModal() {
  document.getElementById("payment-modal-overlay").hidden = true;
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document.getElementById("btn-order-again").addEventListener("click", resetOrderForm);

  // Tutup modal pembayaran: tombol ×, klik area gelap di luar kartu, atau tombol Escape
  document.getElementById("payment-modal-close").addEventListener("click", resetOrderForm);
  ...
  // Tombol "Salin" di sebelah nomor rekening pada popup pembayaran
    ...
    btn.addEventListener("click", () => {
      ...
      if (navigator.clipboard && navigator.clipboard.writeText) {
        navigator.clipboard.writeText(text).then(markCopied).catch(() => {
          console.warn("Gagal menyalin nomor rekening.");
        });
      } else {
